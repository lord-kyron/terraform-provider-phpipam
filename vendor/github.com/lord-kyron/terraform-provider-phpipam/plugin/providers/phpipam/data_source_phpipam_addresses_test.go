...
resource "phpipam_address" "addresses" {
  ...
  custom_fields = {
    CustomTestAddresses  = "terraform-test-multiple"
    CustomTestAddresses2 = "Entry ${var.ip_addresses[count.index]}"
  }
}
`
...
data "phpipam_addresses" "addresses_by_custom_fields" {
  ...
  custom_field_filter = {
    CustomTestAddresses  = "terraform-test-multiple"
    CustomTestAddresses2 = "^Entry [0-9]"
  }
}

output "expected_address_ids" {
  value = ["${phpipam_address.addresses.*.id}"]
}
